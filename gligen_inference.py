...
def load_ckpt(ckpt_path):
    ...
    text_encoder.load_state_dict( saved_ckpt["text_encoder"],strict = False  )  #版本问题
    image_encoder.load_state_dict( saved_ckpt["image_encoder"]  )
    diffusion.load_state_dict( saved_ckpt["diffusion"]  )

    return model, autoencoder, text_encoder, image_encoder, diffusion, config
...
@torch.no_grad()
def run(batch, config, meta, starting_noise=None):
    ...
    context = text_encoder.encode(  [batch["caption"][0]]*config.batch_size  )
    ...
    uc_image = image_encoder(config.batch_size *[ torch.zeros((1,3,224,224)) ])
    ...
    # def check_tensor_devices(tensor_dict):
    #     device_locations = {}
    #     for key, tensor in tensor_dict.items():
    #         if tensor == None:
    #             continue
    #         device_locations[key] = tensor.device
    #     return device_locations
    # print(check_tensor_devices(input))
    # - - - - - start sampling - - - - - #
...
if __name__ == "__main__":
    ...
    # import pdb; pdb.set_trace()
    meta = meta_list[0]
    model, autoencoder, text_encoder, image_encoder, diffusion, config = load_ckpt(meta_list[0]["ckpt"])  #对应自己的模型
    ...
    for i, batch in enumerate(dataloader):
        ...
        gen_image = run(batch,config,meta)   #(ref_image, ref_mask, gt_image.copy(), tar_mask)
